...
class MPC():
    ...
    def buildIneqConstr(self):
        ...
        NoTerminalConstr = np.zeros((np.shape(Mat)[0],
                                     self.n))  # The last state is unconstrained. There is a specific function add the terminal constraints (so that more complicated terminal constrains can be handled)
...
class LMPC(MPC):
    ...
    def addSafeSetIneqConstr(self):
        ...
        self.F_FTOCP = sparse.csc_matrix(
            linalg.block_diag(self.F, np.hstack((-np.eye(self.numSS_Points), np.zeros((self.numSS_Points, self.n))))))
    ...
    def addSafeSetCost(self):
        ...
        self.H_FTOCP = sparse.csc_matrix(
            linalg.block_diag(self.H, np.zeros((self.SS_PointSelectedTot.shape[1], self.SS_PointSelectedTot.shape[1])),
    ...
    def addTerminalComponents(self, x0):
        ...
        if (self.zt[4] - x0[4] > self.predictiveModel.track_length / 2):
            self.zt[4] = np.max([self.zt[4] - self.predictiveModel.track_length, 0])
            self.xLin[4, -1] = self.xLin[4, -1] - self.predictiveModel.track_length
    ...
    def computeCost(self, x, u):
        ...
        for i in range(0, x.shape[0]):
            ...
            elif x[x.shape[0] - 1 - i, 4] < self.predictiveModel.track_length:
    ...
    def addPoint(self, x, u):
        ...
        self.SS[self.it - 1] = np.append(self.SS[self.it - 1], np.array(
            [x + np.array([0, 0, 0, 0, self.predictiveModel.track_length, 0])]), axis=0)
    ...
    def selectPoints(self, it, zt, numPoints):
        ...
        if self.xPred == []:
            Sel_Qfun = self.Qfun[it][indexSSandQfun]
        elif (np.all((self.xPred[:, 4] > self.predictiveModel.track_length) == False)):
            Sel_Qfun = self.Qfun[it][indexSSandQfun]
        elif it < self.it - 1:
            Sel_Qfun = self.Qfun[it][indexSSandQfun] + self.Qfun[it][0]
        else:
            sPred = self.xPred[:, 4]
            predCurrLap = self.N - sum(sPred > self.predictiveModel.track_length)
